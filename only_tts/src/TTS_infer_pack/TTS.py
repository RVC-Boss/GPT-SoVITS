...
# Configuration examples for supported versions (V4 and V2Pro only)
"""
v4:
  ...
  device: cpu
  is_half: false
  t2s_weights_path: GPT_SoVITS/pretrained_models/gsv-v4-pretrained/s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt
...
v2Pro:
  ...
  device: cpu
  is_half: false
  t2s_weights_path: GPT_SoVITS/pretrained_models/v2Pro/s1bert25hz-5kh-longer-epoch=12-step=369668.ckpt
...
class TTS_Config:
    # Only V4 and V2Pro configurations supported
    default_configs = {
        "v4": {
            "device": "cpu",
            "is_half": False,
    ...
    def __init__(self, configs: Union[dict, str] = None):
        ...
        # Default to v4 if custom config not specified
        self.configs: dict = configs_.get("custom", configs_["v4"])
        ...
        # Only support V4 and V2Pro series
        assert self.version in ["v4", "v2Pro", "v2ProPlus"], f"Unsupported version: {self.version}. Only v4, v2Pro, v2ProPlus are supported!"
        self.t2s_weights_path = self.configs.get("t2s_weights_path", None)
        self.vits_weights_path = self.configs.get("vits_weights_path", None)
        self.bert_base_path = self.configs.get("bert_base_path", None)
        self.cnhuhbert_base_path = self.configs.get("cnhuhbert_base_path", None)
        # All supported versions use Korean/English only
        self.languages = ["en", "ko", "auto"]
    ...
    def update_version(self, version: str) -> None:
        if version not in ["v4", "v2Pro", "v2ProPlus"]:
            raise ValueError(f"Unsupported version: {version}. Only v4, v2Pro, v2ProPlus are supported!")
        self.version = version
        # All supported versions use Korean/English only
        self.languages = ["en", "ko", "auto"]
...
class TTS:
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        # Only V2+ versions supported
        hps["model"]["version"] = "v2"  # Default to V2 for all models
        version = hps["model"]["version"]
        # Only support V4 and V2Pro series
        v4_set = {"v4"}
        if model_version in v4_set:
        ...
        # V4 uses SynthesizerTrnV3, V2Pro uses SynthesizerTrn
        if model_version == "v4":
    ...
    def init_vocoder(self, version: str):
        # V4 and V2Pro/V2ProPlus all use the same Generator-based vocoder
        ...
        if version == "v4":
            ...
            state_dict_g = torch.load(
                "%s/GPT_SoVITS/pretrained_models/gsv-v2-pretrained/vocoder.pth" % (now_dir,),
                map_location="cpu",
                weights_only=False,
            )
        ...
        # All supported versions use 32kHz sampling rate
        self.vocoder_configs["sr"] = 32000
        self.vocoder_configs["T_ref"] = 500
        self.vocoder_configs["T_chunk"] = 1000
        self.vocoder_configs["upsample_rate"] = 320
    ...
    def run(self, inputs: dict):
            ...
            inputs (dict):
                {
                    ...
                    "sample_steps": 32,           # int. number of sampling steps for VITS V4 model.
                    "super_sampling": False,       # bool. whether to use super-sampling for audio when using VITS model.
                }
        returns:
        ...
        if split_bucket and speed_factor == 1.0 and not (self.configs.use_vocoder and parallel_infer):
            ...
            print(i18n("当开启并行推理模式时，SoVits V4模型不支持分桶处理，已自动关闭分桶处理"))
            split_bucket = False
        else:
        ...
        if no_prompt_text and self.configs.use_vocoder:
            raise NO_PROMPT_ERROR("prompt_text cannot be empty when using SoVITS")
            ...
            for item in data:
                ...
                if return_fragment:
                    ...
                    yield self.audio_postprocess(
                        ...
                        False,
                        fragment_interval,
                        super_sampling if self.configs.use_vocoder else False,
                    )
                else:
    ...
    def using_vocoder_synthesis(
        ...
        tgt_sr = 32000  # All supported versions use 32kHz
        if ref_sr != tgt_sr:
