...
# ================================
# 文件/文件夹选择辅助函数（使用 tkinter 原生对话框）
# ================================
import threading

def open_folder_dialog(current_path=""):
    """
    打开系统原生文件夹选择对话框。
    在单独的线程中运行 tkinter 以避免阻塞 Gradio。
    """
    ...
    def _open_dialog():
        try:
            ...
            # 设置初始目录
            initial_dir = current_path if current_path and os.path.isdir(current_path) else now_dir
...
def open_file_dialog(current_path="", file_types=None):
    """
    打开系统原生文件选择对话框。
    file_types: 文件类型过滤器，例如 [("List files", "*.list"), ("All files", "*.*")]
    """
    result = {"path": current_path}
    ...
    def _open_dialog():
        try:
            ...
            # 设置初始目录
            initial_dir = os.path.dirname(current_path) if current_path and os.path.exists(os.path.dirname(current_path)) else now_dir
...
def open_list_file_dialog(current_path=""):
    """
    打开文件选择对话框，专门用于选择 .list 文件。
    """
    return open_file_dialog(
        current_path, 
        file_types=[("List files", "*.list"), ("Text files", "*.txt"), ("All files", "*.*")]
    )


def open_audio_file_dialog(current_path=""):
    """
    打开文件选择对话框，专门用于选择音频文件。
    """
    return open_file_dialog(
        current_path,
        file_types=[
...
def get_folder_from_file(file_obj):
    """从上传的文件推断其所在文件夹的路径。"""
    if file_obj is None:
        return ""
    file_path = ""
    ...
    with gr.Tabs():
        with gr.TabItem("0-" + i18n("前置数据集获取工具")):  # 提前随机切片防止uvr5爆内存->uvr5->slicer->asr->打标
            ...
            with gr.Row():
                with gr.Column(scale=3):
                    with gr.Row():
                        slice_inp_path = gr.Textbox(label=i18n("音频自动切分输入路径，可文件可文件夹"), value="", scale=6)
                        slice_inp_btn = gr.Button(i18n("📁 选择文件夹"), scale=1)
                    with gr.Row():
                        slice_opt_root = gr.Textbox(label=i18n("切分后的子音频的输出根目录"), value="output/slicer_opt", scale=6)
                        slice_opt_btn = gr.Button(i18n("📁 选择文件夹"), scale=1)
                    # 按钮点击事件绑定 - 使用原生对话框
                    slice_inp_btn.click(open_folder_dialog, [slice_inp_path], [slice_inp_path])
                    slice_opt_btn.click(open_folder_dialog, [slice_opt_root], [slice_opt_root])
                    with gr.Row():
            ...
            with gr.Row():
                with gr.Column(scale=3):
                    with gr.Row():
                        denoise_input_dir = gr.Textbox(label=i18n("输入文件夹路径"), value="", scale=6)
                        denoise_input_btn = gr.Button(i18n("📁 选择文件夹"), scale=1)
                    with gr.Row():
                        denoise_output_dir = gr.Textbox(label=i18n("输出文件夹路径"), value="output/denoise_opt", scale=6)
                        denoise_output_btn = gr.Button(i18n("📁 选择文件夹"), scale=1)
                    # 按钮点击事件绑定
                    denoise_input_btn.click(open_folder_dialog, [denoise_input_dir], [denoise_input_dir])
                    denoise_output_btn.click(open_folder_dialog, [denoise_output_dir], [denoise_output_dir])
                    with gr.Row():
                ...
                with gr.Column(scale=3):
                    with gr.Row():
                        asr_inp_dir = gr.Textbox(
                            label=i18n("输入文件夹路径"), value="", interactive=True,
                            placeholder=i18n("请输入包含音频文件的文件夹路径"),
                            scale=6
                        )
                        asr_inp_btn = gr.Button(i18n("📁 选择文件夹"), scale=1)
                    with gr.Row():
                        asr_opt_dir = gr.Textbox(label=i18n("输出文件夹路径"), value="output/asr_opt", interactive=True, scale=6)
                        asr_opt_btn = gr.Button(i18n("📁 选择文件夹"), scale=1)
                    # 按钮点击事件绑定
                    asr_inp_btn.click(open_folder_dialog, [asr_inp_dir], [asr_inp_dir])
                    asr_opt_btn.click(open_folder_dialog, [asr_opt_dir], [asr_opt_dir])
                    with gr.Row():
                ...
                with gr.Column(scale=3):
                    with gr.Row():
                        path_list = gr.Textbox(
                            ...
                            value="",
                            interactive=True,
                            placeholder=i18n("请输入 .list 标注文件的完整路径"),
                            scale=6
                        )
                        path_list_btn = gr.Button(i18n("📄 选择文件"), scale=1)
                    # 按钮点击事件绑定
                    path_list_btn.click(open_list_file_dialog, [path_list], [path_list])
                    with gr.Row():
            ...
            with gr.TabItem("1A-" + i18n("训练集格式化工具")):
                ...
                with gr.Row():
                    with gr.Row():
                        ...
                        inp_text_btn = gr.Button(i18n("📄 选择文件"), scale=1)
                    with gr.Row():
                        ...
                        inp_wav_dir_btn = gr.Button(i18n("📁 选择文件夹"), scale=1)
                # 按钮点击事件绑定
                inp_text_btn.click(open_list_file_dialog, [inp_text], [inp_text])
                inp_wav_dir_btn.click(open_folder_dialog, [inp_wav_dir], [inp_wav_dir])
