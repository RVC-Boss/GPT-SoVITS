...
def open1Ba(
    ...
    global p_train_SoVITS, auto_gpt_stop_flag
    auto_gpt_stop_flag = False
...
auto_asr_stop_flag = True
def open_slice(inp_list, opt_root, threshold, min_length, min_interval, hop_size, max_sil_kept, _max, alpha, n_parts):
    if not inp_list or len(inp_list) == 0:
        ...
        return
    global ps_slice, auto_asr_stop_flag
    auto_asr_stop_flag = False
...
def open1abc(
    ...
    global ps1abc, auto_sovits_stop_flag, auto_gpt_stop_flag
    auto_sovits_stop_flag = False
    auto_gpt_stop_flag = False
    inp_text = my_utils.clean_path(inp_text)
    inp_wav_dir = my_utils.clean_path(inp_wav_dir)
    if check_for_existance([inp_text, inp_wav_dir], is_dataset_processing=True):
...
2. 选择一个ASR模型，然后点击`2.开启语音识别`来生成标注文件。如果已事先准备了标注文件，可直接在`标注文件路径`里输入对应的`.list`文件路径。
3. 如果需要对标注文件进行修改，可以点击`3.开启音频标注WebUI`，但这一步是非必须的。
4. 点击`4.开启训练集格式化一键三连`，这一步是为了将数据集整理为特定格式，后续可在`logs`目录里查看。
5. 根据自己显卡的情况调整batch_size，设置合适的训练轮数，然后点击`5.开启SoVITS训练`，可在终端查看训练进度。
6. 同样需要调整batch_size，设置合适的训练轮数，然后点击`6.开启GPT训练`，可在终端查看训练进度。如果显存大于12G，且数据集质量好，建议开启DPO。
...
3. 切换回`微调训练`页面，按照微调训练步骤从头开始。**记得换一个模型名。**""")
    ...
    with gr.Tabs():
        with gr.TabItem(i18n("微调训练")):
            # 语音切分工具
            with gr.Row():
                slice_inp_path = gr.Files(label=i18n("选择一个或多个音频文件"), file_types=["audio"])
                slice_opt_root = gr.Textbox(
                    label=i18n("切分后的子音频的输出根目录"), value="output/slicer_opt", visible=False
                )
            with gr.Row(visible=False):
            ...
            with gr.Row():
                ...
                open_slicer_button = gr.Button(
                    value="1."+process_info(process_name_slice, "open"), variant="primary", visible=True
                )
            ...
            with gr.Row():
                ...
                asr_opt_dir = gr.Textbox(
                    label=i18n("输出文件夹路径"), value="output/asr_opt", interactive=True, visible=False
                )
            ...
            with gr.Row():
                ...
                open_label = gr.Button(
                    value="3."+process_info(process_name_subfix, "open"), variant="primary", visible=True
                )
            ...
            with gr.Accordion(label=i18n("输出logs/实验名目录下应有23456开头的文件和文件夹"), visible=False):
                with gr.Row():
                    with gr.Row():
                        inp_text = gr.Textbox(
                            label=i18n("*文本标注文件"),
                            value=os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "asr_opt", "slicer_opt.list"),
                            interactive=True,
                            scale=10,
                        )
                    with gr.Row():
                        inp_wav_dir = gr.Textbox(
                            ...
                            value=os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "slicer_opt"),
                            interactive=True,
                            placeholder=i18n(
            ...
            def conditional_open_asr(auto_asr, *args):
                global auto_asr_stop_flag
                if auto_asr and not auto_asr_stop_flag:
                    yield from open_asr(*args)
                else:
                    yield ["", {"__type__": "update", "visible": True}, {"__type__": "update", "visible": False}, gr.update(), gr.update(), gr.update()]
            open_slicer_button.click(
                ...
                conditional_open_asr,
                [if_auto_asr, asr_inp_dir, asr_opt_dir, asr_model, asr_size, asr_lang, asr_precision],
            ...
            # 训练集格式化工具
            with gr.Accordion(label="1Aa-" + process_name_1a, visible=False):
                with gr.Row():
                    with gr.Row():
            ...
            with gr.Accordion(label="1Ab-" + process_name_1b, visible=False):
                with gr.Row():
                    with gr.Row():
            ...
            with gr.Accordion(label="1Ac-" + process_name_1c, visible=False):
                with gr.Row():
                    with gr.Row():
            ...
            with gr.Row():
                if_auto_sovits = gr.Checkbox(
                    label=i18n("自动开启SoVITS训练"),
                    value=False,
                    interactive=True,
                    show_label=True,
                )
                if_auto_gpt = gr.Checkbox(
                    label=i18n("自动开启GPT训练"),
                    value=False,
                    interactive=True,
                    show_label=True,
                )
            with gr.Row():
                ...
                button1abc_open = gr.Button(
                    value="4."+process_info(process_name_1abc, "open"), variant="primary", visible=True
                )
                button1abc_close = gr.Button(
                    value="4."+process_info(process_name_1abc, "close"), variant="primary", visible=False
                )
            ...
            def conditional_open1Ba(auto_sovits, *args):
                global auto_sovits_stop_flag
                if auto_sovits and not auto_sovits_stop_flag:
                    yield from open1Ba(*args)
                else:
                    yield ["", {"__type__": "update", "visible": True}, {"__type__": "update", "visible": False}, gr.update(), gr.update()]
            def conditional_open1Bb(auto_gpt, *args):
                global auto_gpt_stop_flag
            ...
            with gr.Row():
                ...
                button1Ba_open = gr.Button(
                    value="5."+process_info(process_name_sovits, "open"), variant="primary", visible=True
                )
            ...
            with gr.Row():
                ...
                button1Bb_open = gr.Button(
                    value="6."+process_info(process_name_gpt, "open"), variant="primary", visible=True
                )
                button1Bb_close = gr.Button(
                    value="6."+process_info(process_name_gpt, "close"), variant="primary", visible=False
                )
